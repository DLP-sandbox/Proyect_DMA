...
def build_snowflake(snowflake: dict) -> go.Figure:
    ...
    # traza de datos, la del borde exterior y la de texto, para que el pop-up sea
    # idéntico se pase el ratón por el vértice o por la ETIQUETA.
    ...
    # Área de fondo (escala máxima). Sus vértices están en la MISMA dirección que
    # cada etiqueta, así que se les da el mismo pop-up: junto con hoverdistance
    # (abajo), pasar el ratón por el texto de la categoría/calificación muestra
    # el tooltip igual que en la gráfica. Los marcadores van invisibles (solo
    # zona de hover) y hoveron="points" evita el hover del relleno ("trace 0").
    ...
    # Score actual (sin texto en vértices — el valor ya está en la etiqueta)
    fig.add_trace(go.Scatterpolar(
        r=values_closed,
        theta=combined_closed,
        customdata=_cd_closed,
        ...
        # grande, con su equivalente sobre 100.
        hovertemplate=_HOVER_TPL,
    ))

    # Etiquetas como TRAZA DE TEXTO dentro del área polar: como ticks del eje
    # quedaban FUERA de la zona que Plotly escucha (no se podía hacer hover sobre
    # el texto) y se cortaban en los extremos. Aquí caen siempre dentro.
    fig.add_trace(go.Scatterpolar(
        r=[23.2] * len(labels),
    ...
    fig.update_layout(
        polar=dict(
            ...
            angularaxis=dict(
                showticklabels=False,   # las etiquetas son ahora una traza de texto
